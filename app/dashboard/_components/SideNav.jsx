...
function SideNav() {
  ...
  const manuList = [
    ...
    { id: 2, name: "Student", icon: GraduationCap, path: "/dashboard/student" },
    { id: 3, name: "Attendance", icon: Hand, path: "/dashboard/attendance" },
    { id: 4, name: "Settings", icon: Settings, path: "/dashboard/settings" }, 
  ];

  return (
    <div className="border shadow-lg h-screen p-5">
      <Image src={"/Logo.svg"} width={80} height={80} />

      <hr className="my-5"></hr>
      {
        manuList.map((menu, index)=>(
          <h2 className="flex items-center gap-3 text-md p-4 text-slate-500 hover:bg-primary hover:text-white rounded-lg cursor-pointer my-3">
            <menu.icon/>
            {menu.name}
          </h2>
        ))
